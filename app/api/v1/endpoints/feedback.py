...
async def get_feedbacks(
    session_id: int,
    user_id: Optional[int] = Query(default=None),  # 지정 시 본인 작성 피드백만
    actor_ids: List[int] = Query(default=[]),  # 여러 개 가능: ?actor_ids=1&actor_ids=2
    db: AsyncSession = Depends(get_db),
):
    """세션의 피드백 조회.
    - user_id 지정 시: 해당 유저가 작성한 피드백만 (작성 화면용)
    - user_id 미지정 시: 세션의 모든 피드백 (다시보기 화면용)
    - actor_ids 지정 시 해당 배우 중 한 명이라도 연결된 피드백만 (OR)
    """
...
async def filter_feedbacks(
    ...
    - user_id 지정 시 해당 유저가 작성한 피드백만 (AND)
